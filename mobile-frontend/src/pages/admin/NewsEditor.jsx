...
const NewsEditor = () => {
  ...
  return (
    <div className="animate-fade-in" style={{ padding: '1rem 0' }}>
      ...
      <div className="editor-header-bar">
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
          <button onClick={() => navigate('/admin/news')}
            style={{ background: 'var(--bg-secondary)', border: '1px solid var(--border-color)', borderRadius: '8px', padding: '0.5rem', cursor: 'pointer', color: 'var(--text-secondary)', display: 'flex' }}>
            <ArrowLeft size={16} />
          </button>
          <div>
            <h1 style={{ marginBottom: '0.15rem' }}>{isEdit ? '✏️ Edit Article' : '📝 Create Article'}</h1>
            <p className="text-secondary" style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' }}>
        ...
        {TABS.map((tab, i) => (
          <button key={tab} onClick={() => setActiveTab(i)}
            style={{
              ...
              color: activeTab === i ? 'var(--primary)' : 'var(--text-muted)', marginBottom: '-2px', transition: 'all 0.2s', flexShrink: 0
            }}>
      ...
      <div className="editor-grid-container" style={{ gridTemplateColumns: aiPanelOpen ? '1fr 320px 320px' : '1fr 320px' }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
          <div className="glass-panel" style={{ padding: '1.5rem', borderRadius: '12px', position: 'relative', minHeight: '500px' }}>
            {/* Tamil Tab */}
            <div style={{ 
              display: 'flex', 

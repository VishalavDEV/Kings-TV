...
public class OpenRouterProvider implements LLMProvider {
    ...
    public String generateContent(String prompt, AiConfiguration config) throws Exception {
        ...
        headers.set("HTTP-Referer", "https://kings24x7.com");
        headers.set("X-Title", "Kings TV CMS");

        Map<String, Object> message = Map.of("role", "user", "content", prompt);
        Map<String, Object> body = new HashMap<>();
        body.put("model", config.getModel() != null && !config.getModel().isBlank() ? config.getModel() : "google/gemini-2.0-flash-exp:free");
    ...
    public boolean testConnection(AiConfiguration config) throws Exception {
        try {
            ...
            throw new Exception("OpenRouter connection test failed: " + e.getMessage(), e);
        }
    }
